...
export function createWallSegment(length: number): THREE.Group {
  const g = new THREE.Group();
  ...
  for (let i = 0; i < merlonCount; i++) {
    const t = (i / (merlonCount - 1) - 0.5) * (length - 0.6);
...
export function createGate(): THREE.Group {
  ...
  part(arch, g, 1.02);
  const door = new THREE.Mesh(new THREE.BoxGeometry(2.6, 2.5, 0.18), wood);
...
export function flashMesh(root: THREE.Object3D, color = 0xffeedd): void {
  root.traverse((obj) => {
    if (obj instanceof THREE.Mesh && obj.material instanceof THREE.MeshStandardMaterial) {
      const prev = obj.material.emissive.getHex();
      const prevI = obj.material.emissiveIntensity;
      obj.material.emissive.setHex(color);
      obj.material.emissiveIntensity = 0.85;
      setTimeout(() => {
        obj.material.emissive.setHex(prev);
        obj.material.emissiveIntensity = prevI;
      }, 90);
    }
  });
}
